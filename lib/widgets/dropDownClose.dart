// ignore_for_file: use_build_context_synchronously, duplicate_ignore, file_names
...
import 'package:changa_changa/new-mobile-2/withdrowalselector.dart';
...
class DropDown extends StatefulWidget {
  ...
  DropDown({super.key, required this.accessToken, required this.channelData});
      ...
      child: DropdownButton2(
        ...
        onChanged: (value) {
          MenuItems.onChanged(context, value! as MenuItem, widget.accessToken,
              widget.channelData.channel_id!, widget.channelData);
        },
...
abstract class MenuItems {
  static const List<MenuItem> firstItems = [withdraw];
  static const List<MenuItem> secondItems = [logout];

  static const withdraw = MenuItem(text: 'Withdraw', icon: Icons.home);
  static const logout = MenuItem(text: 'Close', icon: Icons.logout);
  ...
  static void onChanged(BuildContext context, MenuItem item, String accessToken,
      String channelId, ChannelRead channelData) async {
    switch (item) {
      case MenuItems.withdraw:
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) =>
                Withdrawals(channelData: channelData, accessToken: accessToken),
          ),
        );
        break;
